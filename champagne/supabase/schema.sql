...
create table if not exists public.tastings (
  ...
  -- Naam mag meerdere woorden zijn, maar niet leeg.
  constraint tastings_name_not_empty check (btrim(name) <> ''),
  -- Het oordeel is precies één woord.
  constraint tastings_word_one_word check (word ~ '^\S+$'),
